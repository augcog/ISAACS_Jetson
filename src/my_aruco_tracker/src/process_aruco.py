...
def getCameraPose(pos_x, pos_y, pos_z, roll, pitch, yaw):
    # get x, y, z coords for camera with aruco marker as 0, 0, 0
    pos_x = -pos_x
    pos_y = -pos_y
    pos_z = -pos_z

    # get rotation matrix 
    R_x = np.matrix([[1, 0, 0], [0, np.cos(roll), -np.sin(roll)], [0, np.sin(roll), np.cos(roll)]])
    R_y= np.matrix([[np.cos(pitch), 0, np.sin(pitch)], [0, 1, 0], [-np.sin(pitch), 0, np.cos(pitch)]])
    R_z= np.matrix([[np.cos(yaw), -np.sin(yaw), 0], [np.sin(yaw), np.cos(yaw), 0], [0, 0, 1]])
    ...
    roll = np.arctan(j_z / k_z)
    pitch = np.arctan(-i_z / np.sqrt(1 - i_z ** 2))
    yaw = np.arctan(i_y / i_z)

    return pos_x, pos_y, pos_z, roll, pitch, yaw

def setZEDPose(pos_x, pos_y, pos_z, roll, pitch, yaw):
    try:
        pos_x, pos_y, pos_z, roll, pitch, yaw = getCameraPose(pos_x, pos_y, pos_z, roll, pitch, yaw)
